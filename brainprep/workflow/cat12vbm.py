...
from brainprep.parse_xml import parse_cat12vbm_roi
...
def brainprep_cat12vbm_roi(xml_filenames, outdir):
    """ CAT12 vbm rois workflow. It parses the xml generated by cat12 and 
    summarize the total and region volume data in dataframes, one per cat12 
    atlas.
    ...
    # get all the files given by the regex
    err_msg = "Wrong regex. Please follow the format " \
              "provided in the docstring."
    assert "label" in xml_filenames[0], err_msg
    assert "report" in xml_filenames[1], err_msg
    label_xml_files = glob.glob(xml_filenames[0])
    report_xml_files = glob.glob(xml_filenames[1])
    # list of the current cat12 atlases
    atlases = ["neuromorphometrics", "suit", "thalamic_nuclei", "thalamus"]
    for atlas_name in atlases:
        # create and save a tsv per atlas
        output_file = os.path.join(outdir, f"{atlas_name}_cat12_vbm_roi.tsv")
        rois_tsv = parse_cat12vbm_roi(report_xml_files, label_xml_files,
